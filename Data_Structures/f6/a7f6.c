...
int main(){
 ...
 do{
    ...
    printf("Give name:");
    scanf("%s",AnItem.Data.name); // Διάβασε όνομα πελάτη
    printf("Give amount:");
 ...
 do{ // Επαναληπτικά
    printf("Give code:");
    scanf("%d",&AKey); // Διάβασε κωδικό πελάτη
    SearchHashList(HList,AKey,&Loc,&Pred); // Αναζήτησε τον πελάτη με τον παραπάνω κωδικό
    if(Loc==-1 && Pred==-1){ // Αν δεν βρεθεί ο πελάτης Εμφάνισε τα στοιχεία του
	//Εμφάνισε μήνυμα    
 ...
 do{
    ...
    scanf("%d",&AKey); // Διάβασε κωδικό πελάτη
    SearchHashList(HList,AKey,&Loc,&Pred); // Αναζήτησε τον πελάτη με τον παραπάνω κωδικό
    if(Loc==-1 && Pred==-1){ // Αν δεν βρεθεί ο πελάτης
      //Εμφάνισε μήνυμα     
      printf("DEN YPARXEI EGGRAFH ME KLEIDI %d",AKey);
    }else{ //Αλλιώς
	// Εμφάνισε τα στοιχεία του
        printf("[%d,%s,%d,%d]",HList.List[Loc].RecKey,HList.List[Loc].Data.name,HList.List[Loc].Data.amount,HList.List[Loc].Link);
        printf("\nGive amount:");
        scanf("%d",&AnItem.Data.amount); // Διάβασε το νέο ποσό
        if(HList.List[Loc].Data.amount>=AnItem.Data.amount){ // Αν το ποσό του πελάτη είναι μεγαλύτερο ή ίσο του νέου ποσού
            HList.List[Loc].Data.amount=HList.List[Loc].Data.amount-AnItem.Data.amount; // Ενημέρωσε το ποσό του πελάτη
    }
    }
     printf("\nContinue Y/N:");
      do{
 ...
 do{ // Επαναληπτικά
    ...
    if(Loc==-1 && Pred==-1){ // Αν δεν βρεθεί ο πελάτης
      ...
      if(HList.List[Loc].Data.amount==0){ // Αν ο πελάτης δεν χρωστάει ποσό
            DeleteRec(&HList,AKey); // Διέγραψε τον πελάτη από τη δομή
    ...
    do{ // Επαναληπτικά
	  ...
	  // Εμφάνισε τα στοιχεία του
          printf("[%d,%s,%d,%d]\n",HList.List[Loc].RecKey,HList.List[Loc].Data.name,HList.List[Loc].Data.amount,HList.List[Loc].Link);
             PrintListOfSynonyms(HList,AKey); // Εμφάνισε τη λίστα συνωνύμων για τον κωδικό του πελάτη
        } 
        printf("\nContinue Y/N:");
        do{
        scanf("%c",&ch);
