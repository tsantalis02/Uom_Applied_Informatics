...
void FindMean(int A[][N],int which_row,double *mo_mens,double *mo_womens){
 ...
 {
     if(A[1][j]==0){
        sum1=sum1+A[which_row][j];// Μετρητής, ανάλογα με τον δείκτη, και αποθήκευση στη θέση για άντρα
        counter1++;// Μετρητής αντρών
     }else{
        sum2=sum2+A[which_row][j];/ Μετρητής, ανάλογα με τον δείκτη, και αποθήκευση στη θέση για γυναίκα
        counter2++;// Μετρητής γυναικών
     }
 }
 ...
 *mo_mens=sum1/(counter1*1.0);
     ...
     *mo_mens=0;// Σε περίπτωση που δεν μετρηθεί άνδρας αποθηκεύεται 0
 }
 if(counter2!=0){
 *mo_womens=sum2/(counter2*1.0);
 }else{
     *mo_womens=0; // Σε περίπτωση που δεν μετρηθεί γυναίκα αποθηκεύεται 0
 }
}
